...
object CoProcessFunctionDemo {
  def main(args: Array[String]): Unit = {
    ...
    // 开关流
    val filterSwitches= env.fromCollection(Seq(("sensor_2",10 * 1000L)))// id 为 2 的传感器读数放行 10s
    val forwardedReading: DataStream[SensorReading] = readings.keyBy(_.id).connect(filterSwitches.keyBy(_._1)).process(new ReadingFilter)
  ...
  class ReadingFilter extends CoProcessFunction[SensorReading,(String,Long),SensorReading]{
    ...
    // 没有的话，就初始化一个
    lazy val forwardingEnable = getRuntimeContext.getState(new ValueStateDescriptor[Boolean]("filterSwitch",Types.of[Boolean]))
    // 保存关闭开关的定时器的时间戳
    lazy val disableTimer = getRuntimeContext.getState(new ValueStateDescriptor[Long]("timer",Types.of[Long]))
